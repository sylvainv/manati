...
class Query {
   ...
   * @param restricted whether authorization was set
   * @param authorizationQuery an authorization query
   * @returns {*}
   */
  query(table, data, params, restricted, authorizationQuery) {
    ...
    if (restricted) {
      return this.db.task(function *(task) {
        yield task.any(authorizationQuery.text, authorizationQuery.values);
