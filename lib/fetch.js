...
var build = function(table, params) {
  var promise;
...
var generator = function *(table, params, callback) {
  var query = build();
  db.query(query.text, query.values);
};

module.exports = function Fetch(db, logger) {
  this.db = db;
  this.logger = logger;
  ...
  return function *(table, params, callback) {
    ...
    query = query.toParam();
    promise = db.query(query.text, query.values);
