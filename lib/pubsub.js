...
class PubSub {
    ...
    this.server.on('connection', ws => {
      ...
      ws.on('close', () => {
        ws.channels.forEach(value => {
          // since the socket is closed, there is no need to respond ot the socket we stopped listening successfuly to the channel
          this.unlisten(ws, ws.pgClient, value, false);
        });
      });
  ...
  handleActionMessage(ws, client, message) {
    if (message.action === 'listen') {
      ...
      this.unlisten(ws, client, message, true);
    }
  }
  ...
  unlisten(ws, client, message, respond) {
